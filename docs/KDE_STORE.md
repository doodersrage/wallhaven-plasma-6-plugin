...
When you ship a new release (current: **3.5.1**):
...
3. Paste release notes from `CHANGELOG.md` (3.5.1 section) into the store description / changelog field
...
### 3.5.1 store blurb (copy/paste)

```
v3.5.1 — Reliability: stop rate-limit wallpaper storms, less portrait repeats,
lock-screen sync that reloads, restore wallpaper after sleep/login.
```

### 3.5.0 store blurb (copy/paste)

```
v3.5.0 — Multi-monitor search trust (copy search to other screens), trip warm
progress/cancel with cache fill %, and new pt-BR/nl/pl translations.
```

### 3.4.1 store blurb (copy/paste)

```
v3.4.1 — Hotfix: stop notification floods from retries/filters; isolate
per-screen sync groups so search no longer overwrites every monitor.
```

### 3.4.0 store blurb (copy/paste)

```
v3.4.0 — API auth UX, cache warm/prune/quota, trip mode, plasmoid search
history and purity toggles, notification actions, and settings undo.
```
...
```
v3.3.0 — Offline tag filter and cache browser re-tag, multi-monitor plasmoid
controls, desktop/offline one-click profiles, searchable preset gallery, and
more DE/FR/ES/IT translations.
```
...
```
v3.1.0 — Compact plasmoid with search and cache pin, local-folder depth/exclude/sort,
day-aware smart offline, preset gallery tiles, and lock-screen sync status.
```

### 3.0.0 store blurb (copy/paste)

```
v3.0.0 — Simple/Advanced settings, local folder browse mode, smart offline cache
picks, reduced-motion accessibility, secret-scrubbing exports, typed D-Bus status
helpers, and per-monitor plasmoid control.
```
