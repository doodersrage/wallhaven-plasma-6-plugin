...
## What's new in 3.5.4
- Blank lock-screen pages: non-syncing screens repair the shared greeter image from the SyncLockScreen mirror
...
- Hard-reload wallpapers after sleep/unlock so blank monitors recover
- Clear stuck fade-through-black overlays; blank-frame watchdog
- Heal error/empty/offline blank desktops; queue sync-advance while busy
- Ready-gated transitions; idempotent pause/resume; default nav fan-out
- Honest lock-screen sync status; retry timer no longer drops when busy

## What's new in 3.5.1

- Stop rate-limit cache storms; shared 429 cooldown across monitors
- Less portrait/views repetition; lock-screen sync that actually reloads
- Restore wallpaper after sleep/unlock/login from cache first
...
- Stop notification floods; isolate per-screen search; keep next/pause fan-out

## What's new in 3.4.0

- API auth UX (401 vs outage), key last-4, clear/test key
- Cache warm/prune/quota and trip mode
- Plasmoid search history, saved searches, purity toggles, copy ID/URL
- Notification actions; settings undo

## What's new in 3.3.1

- Fix corrupted API keys that caused persistent Wallhaven 401 errors

## What's new in 3.3.0

- Offline tag filter + cache browser filter/re-tag
- Multi-monitor plasmoid overview with per-screen pause
- Desktop / offline one-click profiles; searchable preset gallery
- More DE/FR/ES/IT translations

## What's new in 3.2.0

- Cache tags power day/night smart offline
- Named local folder playlists
- Plasmoid settings shortcut + richer status tips
- More DE/FR/ES/IT translations
